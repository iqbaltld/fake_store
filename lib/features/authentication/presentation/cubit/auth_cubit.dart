...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<void> checkAuthStatus() async {
    ...
    if (isLoggedIn) {
      ...
      userResult.fold(
        (failure) => emit(const AuthAuthenticated(token: 'logged_in')),
        (user) => emit(AuthAuthenticated(token: 'logged_in', user: user)),
      );
    } else {
  ...
  Future<void> login(String username, String password) async {
    ...
    result.fold(
      ...
      (token) async {
        ...
        final userResult = await getUserDetailsUseCase(1); // FakeStore API demo user
        userResult.fold(
          (failure) => emit(AuthAuthenticated(token: token)),
          (user) => emit(AuthAuthenticated(token: token, user: user)),
        );
      },
    );
  }
